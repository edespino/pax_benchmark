--
-- Phase 6b: Streaming INSERTs - Phase 1 (NO INDEXES) - PROCEDURE Version
...
--   - PROCEDURE allows COMMIT after each batch (resumable execution)
--
...
DROP PROCEDURE IF EXISTS cdr.streaming_insert_phase1_with_commits();

CREATE PROCEDURE cdr.streaming_insert_phase1_with_commits()
LANGUAGE plpgsql
AS $$
DECLARE
    v_batch_num INTEGER;
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '==================================================';
    RAISE NOTICE 'Streaming INSERT simulation (Phase 1) complete!';
...
CALL cdr.streaming_insert_phase1_with_commits();

\echo ''
\echo 'Streaming INSERT simulation (Phase 1) complete!'
\echo ''
