...
\echo '===================================================='
\echo 'Streaming Benchmark - Phase 0: Validation Framework Setup'
\echo '===================================================='
\echo ''
...
DROP SCHEMA IF EXISTS cdr_validation CASCADE;
CREATE SCHEMA cdr_validation;
...
CREATE TABLE cdr_validation.cardinality_rules (
...
INSERT INTO cdr_validation.cardinality_rules (notes) VALUES (
...
CREATE OR REPLACE FUNCTION cdr_validation.validate_bloom_candidates(
    p_schema_name TEXT,
    p_table_name TEXT,
...
BEGIN
    ...
    FROM cdr_validation.cardinality_rules
...
COMMENT ON FUNCTION cdr_validation.validate_bloom_candidates IS
...
CREATE OR REPLACE FUNCTION cdr_validation.calculate_cluster_memory(
    p_table_rows BIGINT
) RETURNS TABLE(
    rows BIGINT,
...
COMMENT ON FUNCTION cdr_validation.calculate_cluster_memory IS
...
CREATE OR REPLACE FUNCTION cdr_validation.detect_storage_bloat(
    p_schema_name TEXT,
    p_baseline_table TEXT,   -- e.g., 'cdr_aoco' (no clustering)
    p_test_table TEXT         -- e.g., 'cdr_pax' (with clustering)
) RETURNS TABLE(
    baseline_table TEXT,
...
COMMENT ON FUNCTION cdr_validation.detect_storage_bloat IS
...
CREATE OR REPLACE FUNCTION cdr_validation.generate_pax_config(
    p_schema_name TEXT,
    p_sample_table TEXT,
    p_target_rows BIGINT,
    p_cluster_columns TEXT DEFAULT NULL  -- e.g., 'call_timestamp,cell_tower_id'
) RETURNS TEXT AS $$
DECLARE
    v_bloom_cols TEXT;
    v_minmax_cols TEXT;
...
BEGIN
    ...
    INTO v_cluster_mem
    FROM cdr_validation.calculate_cluster_memory(p_target_rows);
...
COMMENT ON FUNCTION cdr_validation.generate_pax_config IS
...
\echo '  • cdr_validation.validate_bloom_candidates(...)'
\echo '  • cdr_validation.calculate_cluster_memory(...)'
\echo '  • cdr_validation.detect_storage_bloat(...)'
\echo '  • cdr_validation.generate_pax_config(...)'
\echo ''
\echo 'Next: Phase 1 - Setup CDR schema'
