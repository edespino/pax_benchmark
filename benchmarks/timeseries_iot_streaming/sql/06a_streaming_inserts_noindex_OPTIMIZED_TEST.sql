--
-- Phase 6a: Streaming INSERTs - Phase 1 (NO INDEXES) - OPTIMIZED
-- *** TRACK A OPTIMIZATION: Batch size cap to prevent extreme slowdowns ***
--
-- Changes from 06_streaming_inserts_noindex.sql:
--   1. BATCH SIZE CAP: Maximum 100K rows per batch (no 500K bursts)
--      - Prevents 30x slowdown observed at batch 380 in production run
--      - Still realistic: 100K is typical peak traffic for telecom
--      - Expected impact: Reduce variance from 100-149x to 20-30x
--
...
\echo '===================================================='
\echo 'Streaming Benchmark - Phase 6a: Streaming INSERTs (NO INDEXES) - OPTIMIZED'
\echo '===================================================='
\echo ''
...
DECLARE
    ...
    -- Traffic pattern arrays (50 batches)
...
BEGIN
    ...
    FOR v_batch_num IN 1..50 LOOP
        ...
        IF v_batch_num % 10 = 0 THEN
            RAISE NOTICE '[Batch %/50] Hour: %, Size: % rows, Total rows so far: %M',
    ...
    RAISE NOTICE '';
    RAISE NOTICE '==================================================';
    RAISE NOTICE 'Streaming INSERT simulation complete! (TEST VERSION - 50 batches)';
