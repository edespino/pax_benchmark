--
-- Phase 7c: Streaming INSERTs - Phase 2 - PROCEDURE TEST (10 batches)
-- Quick validation of PROCEDURE with per-batch commits
-- Runtime: ~5-10 minutes (vs 2.7 hours for full 500 batches)
--

\timing on

\echo '===================================================='
\echo 'Streaming Benchmark - Phase 7c: PROCEDURE TEST'
\echo 'Testing per-batch commits with 10 batches only'
\echo '===================================================='
\echo ''
...
DROP TABLE IF EXISTS cdr.streaming_metrics_phase2_test CASCADE;

CREATE TABLE cdr.streaming_metrics_phase2_test (
...
DROP PROCEDURE IF EXISTS cdr.streaming_insert_phase2_test();

CREATE PROCEDURE cdr.streaming_insert_phase2_test()
LANGUAGE plpgsql
AS $$
DECLARE
    v_batch_num INTEGER;
    ...
    v_hour INTEGER;
    v_current_seq BIGINT := 100000000;  -- Start at 100M to avoid conflicts
    ...
    v_table_size BIGINT;
BEGIN
    RAISE NOTICE 'Starting 10-batch test...';
    RAISE NOTICE '';
    ...
    FOR v_batch_num IN 1..10 LOOP
        ...
        INSERT INTO cdr.streaming_metrics_phase2_test VALUES (
...
\echo 'Running 10-batch test...'
\echo ''

CALL cdr.streaming_insert_phase2_test();
...
FROM cdr.streaming_metrics_phase2_test
GROUP BY variant
ORDER BY variant;
